...
from utils import set_seed, load_obj
from train import train_stage, repeated_cross_validation
...
def _suggest_from_space(trial: optuna.Trial, space: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Supports keys: type: [float|loguniform|int|categorical], low/high or min/max, step/choices.
    """
    out: Dict[str, Any] = {}
...
def _make_study_name(base_name: str, space: Dict[str, Any]) -> str:
    # encode the objective + space hash
...
def run_optuna_search(
    ...
    space = OmegaConf.to_container(cfg.optuna.params, resolve=True) if hasattr(cfg.optuna, "params") else {}
    ...
    try:
        print("[Optuna] Study:", study.study_name,
    ...
    # ----- inner helpers -----
    def _run_one_training(local_cfg, trial_params: Dict[str, Any], trial: optuna.Trial) -> Dict[str, Any]:
        """
        ...
        use_cv = bool(getattr(local_cfg, "use_cv", False) or getattr(local_cfg.training, "cross_validation", False))

        if use_cv:
            model, score = repeated_cross_validation(
                local_cfg, csv_path, num_classes=n_class, stage_name=stage, repeats=repeats, trial=trial
            )
        else:
            model, score = train_stage(
                local_cfg, csv_path, num_classes=n_class, stage_name=stage, trial=trial
            )
    ...
    try:
        ...
        if "does not support dynamic value space" in msg:
            ...
            print(f"[Optuna] Detected dynamic space clash. Creating a new study: {alt_name}")
...
def apply_best_params_to_cfg(cfg, best_params: Dict[str, Any], target_nodes: Optional[Dict[str, str]] = None):
...
def _choose_reuse_trial(study, trial_id: int | None):
    ...
    Pick a trial from the study:
      - if trial_id is not None and >= 0, return that trial (must exist & be COMPLETE).
    ...
    if trial_id is not None and trial_id >= 0:
        ...
        if not matches:
            raise RuntimeError(f"[Reuse] Trial {trial_id} not found in '{study.study_name}'.")
        t = matches[0]
        if t.state != optuna.trial.TrialState.COMPLETE:
            raise RuntimeError(f"[Reuse] Trial {trial_id} is not COMPLETE (state={t.state}).")
        return t

    # pick best COMPLETE
    complete = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE and t.value is not None]
    if not complete:
        raise RuntimeError(f"[Reuse] No COMPLETE trials with value found in '{study.study_name}'.")
    # study.direction already encodes maximize/minimize; use study.best_trial if available
    try:
        return study.best_trial
    except Exception:
        # fallback manual best
        reverse = (getattr(study, "direction", None) == optuna.study.StudyDirection.MAXIMIZE)
...
def _apply_trial_params_to_cfg(cfg, params: dict):
    ...
    """
    # Same mapping you use after HPO
    mapping = {
    ...
    # Prefer the centralized utility if you have it imported
    try:
        ...
        if "lr" in params:                 cfg.optimizer.params.lr = float(params["lr"])
        if "weight_decay" in params:       cfg.optimizer.params.weight_decay = float(params["weight_decay"])
        if "batch_size" in params:         cfg.data.batch_size = int(params["batch_size"])

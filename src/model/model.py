...
def build_classifier(cfg: Any, num_classes: int) -> nn.Module:
    ...
    if isinstance(old_head, nn.Sequential):
        in_features = next(m for m in reversed(old_head) if isinstance(m, nn.Linear)).in_features
    else:
...
def _build_model_for_weights(default_cfg: Any, weights_path: str, num_classes: int = 2):
    """
    If the weights include a cfg snapshot, rebuild the model from it (merged over current cfg).
    Otherwise, build from the current cfg as usual.
    """
    ...
    if snap:
        try:
            ...
            print(f"[Load] Snapshot present but rebuild failed ({e}); falling back to current cfg.")
...
class LitClassifier(pl.LightningModule):
    """
    LightningModule wrapping a classifier with training/validation steps.

    Freezing behavior is driven by:
      - cfg.training.freeze_strategy: "none" | "feature_extractor" | "warmup_unfreeze"
      - cfg.training.freeze_backbone_epochs: int (used by "warmup_unfreeze")
    """
    ...
    def on_training_epoch_start(self) -> None:
        self.train_tp.zero_()
        self.train_p.zero_()
        self.train_pred_pos.zero_()
        self.train_correct.zero_()
        self.train_total.zero_()

    def on_validation_epoch_start(self) -> None:
        self.val_tp.zero_()
        self.val_p.zero_()
        self.val_pred_pos.zero_()
    ...
    def configure_optimizers(self):
        ...
        return [optimizer], [{"scheduler": scheduler, "interval": self.cfg.scheduler.step, "monitor": self.cfg.scheduler.monitor}]

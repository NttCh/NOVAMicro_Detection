...
def evaluate_model(model, csv_path: str, cfg, stage: str) -> None:
    ...
    # choose a threshold (fixed 0.5) & preds
    thr = 0.5
...
def predict_folder_to_df(
    ...
    *,
    ...
    ['filename', 'predicted_label', 'probability'].
    ...
    for f in files:
        ...
        if isinstance(x, np.ndarray):
            x = np.transpose(x, (0, 1, 2)) if x.ndim == 3 else x  # no-op for safety
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        for folder_name, df in results_by_folder.items():
            if df is None or df.empty:
                ...
                n = int(len(df))
                c1 = (
                    int((df["predicted_label"] == 1).sum())
                    if "predicted_label" in df.columns
                    else 0
                )
                c0 = (
                    int((df["predicted_label"] == 0).sum())
                    if "predicted_label" in df.columns
                    else 0
                )
            summary_rows.append(
                {"folder": folder_name, "n_images": n, "pred_1": c1, "pred_0": c0}
            )
        ...
        for folder in all_folders:
            ...
            for ckpt_tag, by_folder in all_results.items():
                ...
                if not src.empty and "predicted_label" in src.columns:
                    ...
                    c1 = c0 = 0
                summary.append(
                    {"folder": folder, "ckpt": ckpt_tag, "pred_1": c1, "pred_0": c0}
                )
...
def _run_multi_ckpt_tests(base_model, cfg, dirs, best_ckpt_path: str):
    ...
    for ckpt in ckpt_paths:
        ...
        # Build a fresh model matching the saved weights config (if snapshot present)

...
class MasterValidationMetricsCallback(pl.Callback):
    """
    Write per-epoch metrics to <BASE_SAVE_DIR>/eval/all_eval_metrics.xlsx:
    ...
    def on_validation_epoch_end(self, trainer, pl_module) -> None:  # noqa: ANN001
        ...
        def _tf(name: str) -> Optional[float]:
            v = cm.get(name)
            try:
    ...
    def on_train_end(self, trainer, pl_module) -> None:  # noqa: ANN001
        ...
        for c in ["val_f2", "val_recall", "val_precision", "val_loss", "val_acc", "train_f2", "train_loss"]:

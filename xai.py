#!/usr/bin/env python
"""
xai.py — Grad-CAM++ utilities for CNN classifiers (e.g., ResNet).

Changes requested:
1) XAI output path uses only the folder name, under:
   <eval>/xai_by_ckpt/<ckpt_name>/<folder_name>/
   (no run_id, no 'xai_' prefix).
2) original+overlay ("combo") image shows a prediction-only caption:
   'pred=<label>, p=<prob>' (no filename).
"""
...
def _shorten_with_hash(s: str, maxlen: int = 48) -> str:
    """Shorten string to maxlen with a small md5 tail; keeps it filesystem-safe."""
    s = _slugify(s)
    if len(s) <= maxlen:
        return s
    h = hashlib.md5(s.encode("utf-8")).hexdigest()[:6]
    return f"{s[:maxlen-7]}_{h}"

def _win_long(path: str) -> str:
    """Return a Windows extended-length path if on Windows; otherwise the same path."""
    if os.name == "nt":
...
# CAM helpers
# =========================
def _generate_cam_fused(model, x, device, layer_paths: List[str], cam_cls) -> np.ndarray:
    """Average CAMs from multiple layers (e.g., ['layer2','layer3','layer4'])."""
    cams = []
    for lp in layer_paths:
        gc = cam_cls(model, target_layer=lp, device=device)
        cam = gc.generate(x, class_idx=None)
        gc.close()
        cam = (cam - cam.min()) / (cam.max() + 1e-8)
        cams.append(cam)
    cam_fused = np.mean(cams, axis=0)
    return (cam_fused - cam_fused.min()) / (cam_fused.max() + 1e-8)

def _tta_cam(generate_fn: Callable[[torch.Tensor], np.ndarray], x: torch.Tensor, H: int, W: int) -> np.ndarray:
    """Flip/rotate test-time augmentation for CAMs; returns (H,W) CAM."""
    cams = []
    ...
    cams.append(norm_resize(generate_fn(x)))  # identity
    xr = torch.flip(x, dims=[3]);  c = generate_fn(xr); c = np.flip(c, axis=1); cams.append(norm_resize(c)) # hflip
    xr = torch.flip(x, dims=[2]);  c = generate_fn(xr); c = np.flip(c, axis=0); cams.append(norm_resize(c)) # vflip
    xr = torch.rot90(x, k=1, dims=[2,3]); c = generate_fn(xr); c = np.rot90(c, 3); cams.append(norm_resize(c)) # rot90

    cam = np.mean(cams, axis=0)
    return (cam - cam.min()) / (cam.max() + 1e-8)

def _smooth_cam(generate_fn: Callable[[torch.Tensor], np.ndarray], x: torch.Tensor, H: int, W: int,
                n: int = 6, sigma: float = 0.05) -> np.ndarray:
...
def _mask_scale_bar_inplace(rgb: np.ndarray, w_frac: float = 0.18, h_frac: float = 0.08) -> None:
    """White-out a bottom-right rectangle to remove the scale bar (edit in place)."""
    h, w = rgb.shape[:2]
...
def _draw_caption(img: np.ndarray, text: str, *, color=(255, 255, 255),
                  top_offset: int = 14, left_offset: int = 8) -> np.ndarray:
    out = img.copy()
...
class GradCAMPP:
    ...
    def __init__(self, model: nn.Module, target_layer: str, device: str = "cpu"):
        ...
        if target_layer in (None, "", "auto"):
            layer = _find_last_conv(root)
        else:
            layer = _get_module_by_path(root, target_layer)

        self._feats, self._grads = None, None

        def f_hook(_, __, out): self._feats = out.detach()
        def b_hook(_, grad_in, grad_out): self._grads = grad_out[0].detach()
...
def explain_image_path(
    ...
    *,
    ...
    # combo caption mode: "pred_only" by default (no filename)
    combo_caption_mode: str = "pred_only",  # options: "none", "pred_only", "full"
    pred_prob_digits: int = 6,              # p= with 6 decimals
    # NEW: also save Original|Overlay|Heatmap triptych
    save_triple_panel: bool = True,
) -> dict:
    """Generate CAM visualizations for one image (Grad-CAM++)."""
    ...
    # NEW: folder for triple panel [original|overlay|heatmap]
    triple_dir  = (output_dirs or {}).get("original_overlay_heatmap",
                    os.path.join(save_dir, "original_overlay_heatmap"))

    for d in (heat_dir, overlay_dir, panel_dir, evid_dir, combo_dir, triple_dir):
        os.makedirs(d, exist_ok=True)

    # ---- load RGB (clean), optional scale-bar masking ----
    ...
    # ---- transform -> tensor (no captions drawn before inference!) ----
    ...
    # ---- predict class/prob on CLEAN image ----
    ...
    # ---- captions (strings only; drawing happens later) ----
    stem_for_caption = (file_prefix.strip() if isinstance(file_prefix, str) and file_prefix.strip()
    ...
    # ---- CAM (with optional fusion/TTA/Smooth) ----
    H, W = h0, w0
    def single_cam(z: torch.Tensor) -> np.ndarray:
        if fuse_layers:
    ...
    base_cam = cv2.resize(single_cam(x), (W, H), interpolation=cv2.INTER_LINEAR)
    cams = [(base_cam - base_cam.min()) / (base_cam.max() + 1e-8)]
    if tta:
        cams.append(_tta_cam(single_cam, x, H, W))
    if smooth:
        cams.append(_smooth_cam(single_cam, x, H, W, n=smooth_n, sigma=smooth_sigma))
    cam = np.mean(cams, axis=0)
    cam = (cam - cam.min()) / (cam.max() + 1e-8)

    # ---- filenames (shortened safely) ----
    ...
    # Precompute color heatmap for reuse
    heat_rgb = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)[:, :, ::-1]

    # ---- heatmap (caption can be toggled with caption_heatmap) ----
    if save_heatmap:
    ...
    # ---- original + overlay side-by-side (ONE caption) ----
    if save_original_overlay and overlay_img is not None:
        ...
        if combo_caption_enabled:
            side_by_side = _draw_caption(side_by_side, combo_caption_text)

        combo_path = os.path.join(combo_dir, f"{stem}.png")
        _ensure_parent_dir(combo_path)
        Image.fromarray(side_by_side).save(_win_long(combo_path))

    # ---- NEW: original + overlay + heatmap (ONE caption) ----
    if save_triple_panel and overlay_img is not None and heat_rgb is not None:
        left   = rgb
        middle = overlay_img
        right  = heat_rgb

        # ensure same height for all
        ...
        left   = resize_h(left)
        middle = resize_h(middle)
        right  = resize_h(right)

        triple = np.hstack([left, middle, right])
    ...
    # ---- panel (top-K evidence boxes, captioned with *full* text) ----
    ...
    return {
        ...
        "original_overlay_heatmap_path": triple_path,  # NEW
    }
...
def explain_folder(
    ...
    *,
    ...
    """Run Grad-CAM++ over all images in a folder (recursively)."""
    ...
    for p in paths:
        rec = explain_image_path(
            ...
            tta=tta, smooth=smooth, smooth_n=smooth_n, smooth_sigma=smooth_sigma,
            mask_scale_bar=mask_scale_bar, mask_rect_wh=mask_rect_wh,
...
def _maybe_run_xai(model, transform, folder, dirs, cfg_xai, ckpt_tag: str | None = None):
    """
    Run XAI with Grad-CAM++ if enabled.

    Output path now:
      <eval>/xai_by_ckpt/<ckpt_tag>/<folder_name>/
    (Only the folder name — mirrors multi-prediction style but under xai.)
    """
    ...
    target_layer   = getattr(cfg_xai, "target_layer", "auto") or "auto"  # str or list[str]
    ...
    # combo caption mode: default "pred_only" (as requested)
    ...
    explain_folder(
        ...
        caption_overlay=caption_overlay,
        caption_panel=caption_panel,
        combo_caption_mode=combo_caption_mode,   # "pred_only" => no filename on combo
        pred_prob_digits=pred_prob_digits,
    )
